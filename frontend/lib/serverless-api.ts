...
export async function updateFunction(id: string, body: Partial<{
  name: string;
  description: string;
  code: string;
  runtime: "javascript" | "typescript";
...
export async function executeFunction(id: string, payload?: unknown): Promise<ExecutionResult> {
  return api<ExecutionResult>(`/serverless/functions/${id}/execute`, { method: "POST", body: payload ?? {} });
}

export async function getFunctionLogs(id: string, limit = 50, offset = 0): Promise<ExecutionLog[]> {
  return api<ExecutionLog[]>(`/serverless/functions/${id}/logs?limit=${limit}&offset=${offset}`);
}
...
export async function createTrigger(funcId: string, body: {
  type: "http" | "cron";
  httpMethod?: string;
  cronExpr?: string;
  enabled?: boolean;
}): Promise<FunctionTrigger> {
  return api<FunctionTrigger>(`/serverless/functions/${funcId}/triggers`, { method: "POST", body });
}

export async function updateTrigger(funcId: string, triggerId: string, body: Partial<{
  httpMethod: string;
  cronExpr: string;
  enabled: boolean;
}>): Promise<FunctionTrigger> {
  return api<FunctionTrigger>(`/serverless/functions/${funcId}/triggers/${triggerId}`, { method: "PUT", body });
}
